...
class RescheduleController extends Controller
{
    ...
    public function clientInterface(string $token)
    {
        ...
        foreach ($offeredSlots as $slot) {
            $slot->formatted_date = Carbon::parse($slot->slot->slot_start_time)->format('d M Y');
            $slot->formatted_time = Carbon::parse($slot->slot->slot_start_time)->format('H:i');
        }

        // Get time remaining until expiration
        $expiresAt = Carbon::parse($reschedule->expires_at);
    ...
    public function selectSlot(Request $request, string $token)
    {
        ...
        if ($action === 'accept') {
            ...
            $newDate = Carbon::parse($selectedSlot->slot_start_time)->format('d M Y');
            $newTime = Carbon::parse($selectedSlot->slot_start_time)->format('H:i');
