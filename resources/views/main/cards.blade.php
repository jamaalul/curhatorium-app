...
<section
    class="w-screen overflow-x-hidden px-4 py-8 bg-cover flex flex-col items-center justify-center shadow-inner h-fit"
    style="background-image: url('{{ asset('images/background.webp') }}');" id="cards-section">
    ...
    <button id="shuffle-cards"
        class="bg-[#9acbd0] hover:bg-[#48a6a6] text-white py-2 px-12 rounded mt-4 flex items-center gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor" class="size-5">
            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function fetchAndRenderCards() {
            try {
                ...
                gridCardsContainer.innerHTML =
                    `<p class="text-red-500">Failed to load cards. Please try again later.</p>`;
        ...
        function renderSlider(cards) {
            ...
            cards.forEach((card, index) => {
                swiperCardsContainer.innerHTML +=
                    `<div class="swiper-slide">${getCardHTML(card, index)}</div>`;
            });
        ...
        function renderGrid(cards) {
            ...
            gridCards.forEach((card, index) => {
                gridCardsContainer.innerHTML +=
                    `<div class="w-full" style="max-width: 240px;">${getCardHTML(card, index)}</div>`;
            });
            ...
            container.querySelectorAll('.card-flip').forEach(card => {
                card.addEventListener('click', function() {
                    ...
                    container.querySelectorAll('.card-response-form').forEach(form => {
                        ...
                        form.classList.add('card-hidden', 'opacity-0',
                        '-translate-y-5');
                    });
                    ...
                    if (!isFlipped) {
                        ...
                        const form = container.querySelector(
                            `.card-response-form[data-card-index="${cardIndex}"]`);
                        if (form) {
            ...
            container.querySelectorAll('.card-response-form').forEach(form => {
                ...
                form.addEventListener('submit', function(e) {
                    ...
                    if (textarea.value.trim() !== '') {
                        ...
                        'Terima kasih sudah berbagi. Kamu luar biasa!';
