...
<body class="bg-gray-100">
    <div class="flex flex-col md:flex-row h-screen">
        ...
        <aside class="w-full md:w-64 bg-white shadow-md flex-shrink-0">
            ...
            <nav id="mobile-menu" class="mt-2 md:mt-6 hidden md:block">
                <a href="#schedule" id="nav-schedule"
                ...
                <div id="schedule-section">
                                    ...
                                    @forelse($pendingBookings as $booking)
                                        <tr class="bg-white border-b">
                                            ...
                                            <td class="px-4 py-4 flex flex-col sm:flex-row gap-2">
                                                <form action="{{ route('professional.booking.accept', $booking) }}"
                                                    method="POST">
                                                    @csrf
                                                    <button type="submit"
                                                        class="px-3 py-1 text-xs font-medium text-center text-white bg-green-600 rounded-lg hover:bg-green-700">Accept</button>
                                                </form>
                                                <form action="{{ route('professional.booking.decline', $booking) }}"
                                                    method="POST">
                                                    @csrf
                                                    <button type="submit"
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
                        <div class="lg:col-span-2 bg-white p-4 md:p-6 rounded-lg shadow-md relative">
                            <div id="calendar-management-overlay"
                                class="absolute inset-0 bg-red-500 bg-opacity-10 border-2 border-red-500 rounded-lg hidden items-center justify-center">
                                <p class="text-red-700 font-semibold text-base md:text-lg w-full text-center pt-4">Mode
                                    Hapus Aktif: Klik slot untuk menghapus</p>
                            </div>
                            <div class="flex flex-col md:flex-row justify-between md:items-center mb-4">
                                <h3 class="text-lg md:text-xl font-bold mb-2 md:mb-0">Kalender Jadwal</h3>
                                <div class="flex items-center">
                                    <label for="manage-schedule-toggle"
                                        class="mr-2 text-xs md:text-sm font-medium text-gray-900">Hapus Jadwal</label>
                        ...
                        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
                            ...
                            <form id="scheduleForm" class="space-y-4">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label for="end_time"
                                            class="block text-xs md:text-sm font-medium text-gray-700">Sampai
                                            Jam</label>
                                ...
                                <button type="submit"
                                    class="w-full bg-[#48A6A6] text-white py-2 px-4 rounded-md hover:bg-[#357979] text-sm">Buat
                                    Slot Jadwal</button>
                            </form>
                                ...
                                <tbody>
                                    @forelse($recentSessions as $session)
                                        <tr class="bg-white border-b">
                                            ...
                                            <td class="px-4 py-4">
                                                {{ \Carbon\Carbon::parse($session->slot_start_time)->format('d M Y, H:i') }}
                                            </td>
                                            <td class="px-4 py-4">
                                                <span
                                            ...
                                            <td class="px-4 py-4">
                                                @if ($session->status === 'booked')
                                                    @php
                                                        ...
                                                        $isVideo =
                                                            stripos(
                                                                $session->consultation->consultation_type,
                                                                'video',
                                                            ) !== false;
                                                    @endphp
                                                    <button
                                                        @if ($isVideo) onclick="if(!this.disabled) window.location.href='/professional/video/{{ $session->consultation->room }}'"
                                                    @else
                                                        onclick="if(!this.disabled) window.location.href='/professional/chat/{{ $session->consultation->room }}'" @endif
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="px-4 py-4 text-center text-gray-500">Tidak ada
                                                sesi terbaru.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
                            ...
                            <form id="passwordForm" class="space-y-4">
                                ...
                                <button type="submit"
                                    class="w-full bg-[#48A6A6] text-white py-2 px-4 rounded-md hover:bg-[#357979]">Ubah
                                    Password</button>
                            </form>
    ...
    <div id="delete-modal"
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md m-4 p-4 md:p-6">
            ...
            <p id="delete-modal-text" class="mb-6">Apakah Anda yakin ingin menghapus slot jadwal yang tersedia ini?
            </p>
            <div class="flex justify-end gap-4">
                <button id="cancel-delete-btn"
                    class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Batal</button>
                <button id="confirm-delete-btn"
                    class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">Ya, Hapus</button>
            </div>
        </div>
    </div>
